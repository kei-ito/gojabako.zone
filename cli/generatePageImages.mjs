...
const cardContent = applyPadding(card, {
  left: 40,
  right: 40,
  top: 40,
  bottom: 40,
...
const generate = async (page) => {
  ...
  const versionDirectory = path.join(
    pathDirectory,
    getHash(JSON.stringify({ page, version }))
      .toString('base64url')
      .slice(0, 8),
  );
  ...
  console.info(
    `generated: ${path.relative(publicDirectory, dest)} (${page.pathname})`,
  );
  return dest;
};
...
const draw = async (page) => {
  ...
  nodeCanvas.registerFont('/Library/Fonts/ヒラギノUD角ゴ StdN W4.otf', {
    family: 'HiraginoW4',
  });
  nodeCanvas.registerFont('/Library/Fonts/ヒラギノUD角ゴ StdN W6.otf', {
    family: 'HiraginoW6',
  });
  nodeCanvas.registerFont('/Library/Fonts/ヒラギノ角ゴ StdN W8.otf', {
    family: 'HiraginoW8',
  });
...
const drawLogo = async (ctx) => {
  ...
  ctx.translate(
    cardContent.cx - logoWidth / 2,
    card.bottom + card.padding.bottom / 2 - logoHeight / 2,
  );
...
const drawQrCode = async (ctx) => {
  ...
  ctx.beginPath();
  ctx.rect(
    cardContent.right - qrCodeMaxSize,
    cardContent.bottom - qrCodeMaxSize,
    qrCodeMaxSize,
    qrCodeMaxSize,
  );
  ctx.closePath();
