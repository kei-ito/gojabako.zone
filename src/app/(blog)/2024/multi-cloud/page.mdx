...
このブログをCloud Run, AWS Amplify, Netlify, Vercelでホストするようにしました。DNS (Route 53) でリクエストを振り分けています。
...
Route 53でヘルスチェックを設定し正常でないホスト先を返さないようにしました。

1. middleware.ts で GET /health に 200 を返すようにします。
2. ホスト先の `https://mainブランチデプロイ先/health` が動いているか確認します。
...
AWS以外のエンドポイントにHTTPSでヘルスチェックすると1ホスト 2.75USD/月 です。ちょっともったいないのでCloudWatchアラームで節約できるか試してみました。
