...
例えば`*.mts`をTypeScriptでロードするために以下のような設定をしたとしましょう。`MyTsLoader`は[`{"type":"module"}`](https://nodejs.org/api/packages.html#type)なパッケージとします。

```javascript webpack.config.js
const webpackConfig = {
  module: { rules: [{ test: /\.mts$/, loader: 'MyTsLoader' }] },
};
```
...
- 画像を生成しているコードはこちら: [`/app/cover/[[...path]]/route.tsx`](https://github.com/gjbkz/gojabako.zone/blob/main/src/app/cover/%5B%5B...path%5D%5D/route.tsx)
  - フォントは [@fontsource/noto-sans-jp](https://www.npmjs.com/package/@fontsource/noto-sans-jp) のフォルダから`readFile`します。
  - 文字出力の際に提供したフォントにない文字があるとエラーになります。
  - [adobe-blank](https://github.com/adobe-fonts/adobe-blank)を渡しておけばエラーにはならなくなりそうです。
  - `runtime`を [edge](https://nextjs.org/docs/pages/api-reference/edge) にするのはファイルの扱いがややこしいのでおすすめしません。
...
- `export metadata = {...}`の中に`openGraph`を追加する
  ...
  - `node.data.estree`にJavaScriptのASTがあるので`metadata`を`export`するExportNamedDeclarationがあればそれをいじる
  - `node.value`にコードが文字列で入っていますがそちらを変更してもmetadataは変更されません。
  - 今考えると`node.value`をいじって[^transform]`node.data.estree`を更新する方が楽でした。

[^id-offset]: idを`position:absolute`で実際の要素よりも上に表示した要素につけるとリンクから飛んできたときにヘッダーで隠れないようにできます。
[^transform]: 最初の`metadata = {`の直後に`openGraph:{...},`を入れます。

機能追加は以下のような関数を`next.config.mjs`の`remarkPlugins`と`rehypePlugins`に追加すれば動きます。

```typescript rehypePlugin.mts
