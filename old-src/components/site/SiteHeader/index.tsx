...
const meilisearchHost = ensure(
  process.env.NEXT_PUBLIC_MEILISEARCH_HOST,
  isString,
);
const meilisearchApiKey = ensure(
...
export const SearchResult = ({
  ...
  if (candidates.length === 0) {
    return (
      <div className={classnames(style.searchResult, style.empty)}>
  ...
  return (
    <div className={style.searchResult}>
      {candidates.map((item) => (
        <SearchResultItem {...item} key={item.pathname} />
      ))}
    </div>
  );
};
